...
class StoreStorageProxy {
  ...
  Future<ResultInterface> openOnlineStore(OnlineStoreDTO store) async {
    ...
    if (!storeOwnerRes.getTag()) {
      ...
      storeOwner =
          StoreOwnerModel(onlineStoreModel: onlineWithProducts, storeOwnerModelOnlineStoreModelId: onlineWithProducts.id);
  ...
  Future<List<StoreDTO>> fetchAllStores() async {
    try {
      ...
      List<PhysicalStoreDTO> physicalDtos = await convertPhysicalStoreModelToDTO(physicalStores);
  ...
  Future<List<ProductDTO>> fetchStoreProducts(String storeId) async {
    ...
    return products
        .map((e) => ProductDTO(e.name, e.price, jsonDecode(e.categories).cast<String>(), e.imageUrl!, e.description!))
        .toList();
  }
  ...
  Future<ResultInterface> createProductForOnlineStore(ProductDTO productDTO, String onlineStoreModelID) async {
    try {
      StoreProductModel productModel = StoreProductModel(
          ...
          categories: productDTO.categories[0].isEmpty ? jsonEncode(["Default"]) : jsonEncode(productDTO.categories),
