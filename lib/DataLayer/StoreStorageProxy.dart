...
class StoreStorageProxy {
  ...
  Future<ResultInterface> openOnlineStore(OnlineStoreDTO store, [String? storeID, DateTime? lastViewPurchase]) async {
    ...
    if (store.imageFromPhone != null) {
      ...
      await uploadPicture(id, store.imageFromPhone); // uploading the picture to s3
  ...
  Future<ResultInterface> updateOnlineStoreProducts(List<ProductDTO> products, String storeID) async {
    ...
    if (productsModels.isNotEmpty) {
      for (StoreProductModel prod in productsModels) {
        var p = await deletePicture(prod.id);
        if (!p.getTag()) return p;
  ...
  Future<ResultInterface> deleteOnlineStoreProducts(String storeID) async {
    List<StoreProductModel> products = await Amplify.DataStore.query(StoreProductModel.classType,
        where: StoreProductModel.ONLINESTOREMODELID.eq(storeID));
    ...
    return new Ok("Deleted all products");
  }
  ...
  Future<ResultInterface> deleteOnlineStore(String id) async {
    try {
      ...
      var p = await deletePicture(id); // in s3 - for store picutre
      if (!p.getTag()) throw TimeoutException(p.getMessage());
      var b = await deletePicture("$id-qrcode"); //in s3
      if (!b.getTag()) throw TimeoutException(b.getMessage());
      var r = await deleteOnlineStoreProducts(stores[0].id);
      if (!r.getTag()) throw TimeoutException(r.getMessage());
      ...
      FLog.error(text: e.toString(), stacktrace: StackTrace.current);
      return new Failure("Please check your internet connection. Your store was not deleted.", id);
    } on Exception catch (e) {
      FLog.error(text: e.toString(), stacktrace: StackTrace.current);
