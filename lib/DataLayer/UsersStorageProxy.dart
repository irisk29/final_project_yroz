...
class UsersStorageProxy {
  ...
  Future<Tuple2<UserModel, bool>> createUser(
    ...
    {
      UserModel userModel =
          UserModel(email: email, name: name, imageUrl: imageUrl);
    ...
    return Tuple2<UserModel, bool>(
        await createFullUser(user, user.name, user.imageUrl), false);
  }
  ...
  Future<ResultInterface> addFavoriteProduct(String prodID) async {
    ...
    if (favoriteProd != null) {
      List<String> fav =
          (jsonDecode(user.favoriteProducts!) as List<dynamic>).cast<String>();
      ...
      return new Ok(
          "Added succssefully product $prodID to user's favorite", fav);
    }
    var updatedUser = user.copyWith(
    ...
    FLog.info(text: "Added succssefully product $prodID to user's favorite");
    return new Ok(
        "Added succssefully product $prodID to user's favorite", [prodID]);
  }
  ...
  Future<ResultInterface> removeFavoriteProduct(String prodID) async {
    ...
    if (favoriteProd != null) {
      ...
      FLog.error(
          text:
              "There is no favorite stores list from current user ${user.id}");
      return new Failure(
          "There is no favorite stores list from current user ${user.id}",
          null);
    }
    FLog.error(
        text:
            "There is no favorite products list from current user ${user.id}");
    return new Failure(
        "There is no favorite products list from current user ${user.id}",
        null);
  }
  ...
  CartProductDTO convertStoreProductToCartProduct(
      ProductDTO productDTO, double quantity) {
  ...
  Future<ResultInterface> getOrCreateUserShoppingBagPerStore(
    ...
    List<ShoppingBagModel> shoppingBags = await Amplify.DataStore.query(
        ShoppingBagModel.classType,
        where: ShoppingBagModel.USERMODELID.eq(userID).and(
            ShoppingBagModel.SHOPPINGBAGMODELONLINESTOREMODELID.eq(storeID)));
    if (shoppingBags.isEmpty) {
      ...
      ShoppingBagModel shoppingBagModel = new ShoppingBagModel(
          usermodelID: userID, shoppingBagModelOnlineStoreModelId: storeID);
  ...
  Future<ResultInterface> convertShoppingBagModelToDTO(
    ...
    ShoppingBagDTO shoppingBag = ShoppingBagDTO(shoppingBagModel.usermodelID,
        shoppingBagModel.shoppingBagModelOnlineStoreModelId!);
    List<CartProductDTO> shoppingBagProductsDTO = vals
        .map((e) => convertCartProductModelToDTO(
            e, shoppingBagModel.shoppingBagModelOnlineStoreModelId!))
        .toList();
  ...
  Future<ResultInterface> updateProductQuantityInBag(ProductDTO productDTO,
      String storeID, double quantity, String userID) async {
    List<ShoppingBagModel> shoppingBags = await Amplify.DataStore.query(
        ShoppingBagModel.classType,
        where: ShoppingBagModel.USERMODELID.eq(userID).and(
